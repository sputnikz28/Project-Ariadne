...
def dwarves(cfg,ctx):
    out=[]; est=ctx['estatisticas']; names=['Barbas de Ferro','Cristal Azul','Forja Negra']
    for name in names[:cfg.getint('ANOES','numero_clas')]:
        ...
        for ns in cnums:
            ...
            if len(cart)>=cfg.getint('ANOES','chaves_por_cla'): break
        out.append({'nome':name,'lider':'Rei '+random.choice(['Thorin','Borin','Dain']),'pool':sorted(pool[:20]),'estrelas_pool':sorted(ep[:4]),'carteira':cart})
    return out
...
def faeries(cfg,ctx):
    ...
    for i in range(cfg.getint('FADAS','quantidade')):
        ...
        nums=[n for n,_ in rank]; weights=[s for _,s in rank]; key=None
        for _ in range(2000):
            c=list(dict.fromkeys(random.choices(nums,weights=weights,k=12)))
            if len(c)<5: continue
            ns=sorted(random.sample(c,5)); pares=sum(n%2==0 for n in ns)
            if any(n<=10 for n in ns) and set(ns)&set(est['quentes']) and set(ns)&set(est['frios']) and 100<=sum(ns)<=170 and pares in (2,3): key=ns; break
        if key is None: key=sorted(random.sample(range(1,51),5))
...
def melforks(cfg,ctx):
    ...
    est=ctx['estatisticas']; tam=cfg.getint('MELFORKS','populacao_chaves'); pop=[normalize(random.sample(range(1,51),5),random.sample(range(1,13),2)) for _ in range(tam)]
    for _ in range(cfg.getint('MELFORKS','geracoes_chaves')):
        ...
        while len(nova)<tam:
            a,b=random.sample(elite,2); pn=list(dict.fromkeys(a[0]+b[0])); pe=list(dict.fromkeys(a[1]+b[1])); nova.append(normalize(random.sample(pn,5),random.sample(pe,2)))
        pop=nova
...
def werewolves(cfg,ctx):
    ...
    return {'ativo':True,'simulacoes':sims,'finalistas':[{'nome':f'Fenrir-{i+1}','tipo':'Lobisomem','fitness':f,'chave':normalize(list(n),list(e))} for i,(f,n,e) in enumerate(top)]}
...
def treefolks(cfg,ctx):
    ...
    for i in range(cfg.getint('TREEFOLKS','quantidade')):
        ...
        out.append({'nome':f'Raiz-{i+1}','tipo':'Treefolk','modelo':random.choice(['Random Forest','Rede Neural','LSTM','Bayesiano']),'treino':tr,'teste':te,'fantasma':fa,'peso':max(.02,1-fa),'chave':normalize([n for n,_ in rn[:5]],[e for e,_ in re[:2]])})
    return out


def superiors(ctx):
    est=ctx['estatisticas']; vis=[]
    for i in range(2): vis.append({'nome':f'Mago-{i+1}','tipo':'Mago','chave':normalize(random.sample([n for n,_ in est['atrasados'][:5]],2)+random.sample(est['quentes'],3),random.sample(est['estrelas_quentes'],2))})
    for i in range(2): vis.append({'nome':f'Druida-{i+1}','tipo':'Druida','chave':normalize(random.sample(est['quentes'],5),random.sample(est['estrelas_quentes'],2))})
    for i in range(2): vis.append({'nome':f'Djinn-{i+1}','tipo':'Djinn','chave':normalize([max(1,min(50,n+random.choice([-5,-3,-1,1,3,5]))) for n in ctx['historico'][-1]['numeros']],[max(1,min(12,e+random.choice([-2,-1,1,2]))) for e in ctx['historico'][-1]['estrelas']])})
    vn,ve=Counter(),Counter()
    for v in vis: vn.update(v['chave'][0]); ve.update(v['chave'][1])
    return vis,{'nome':'Aion','tipo':'Deus','chave':normalize([n for n,_ in vn.most_common(5)],[e for e,_ in ve.most_common(2)])}
