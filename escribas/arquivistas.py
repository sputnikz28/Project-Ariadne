...
def inventariar_era(era, resumo_execucao):
    ...
    reflexos = listar_json("esquadrao_negro/biblioteca_sombria/reflexos_corrompidos")
    roubadas = listar_json("esquadrao_negro/biblioteca_sombria/reliquias_roubadas")
    missoes = listar_json("ordem_elfica/arquivo_missoes")
    livros = listar_json("amuletos/livros")
    lendas = ler("lendas/livro_personagens_lendarias.json", {"personagens": []}).get("personagens", [])
    individuos = ler("dados/todos_individuos.json", [])
