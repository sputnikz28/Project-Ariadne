...
from esquadrao_negro.persistencia import (
    REFLEXOS, ROUBADAS, carregar_grimorio, guardar_grimorio, ler, guardar
)
from artefactos.arca import guardar as guardar_reliquia
...
def executar_missoes(config, ninjas, eventos):
    estado_path = Path("ordem_elfica/estado_ordem.json")
    ...
    for tipo, path in alvos[:max_missoes]:
        ...
        defesa = min(0.90, 0.25 + grimorio.get("nivel", 1) * 0.05)
        ...
        if sucesso:
            ...
            if tipo == "LIVRO":
                ...
                destino = Path("amuletos/cache/conhecimento_recuperado") / path.name
                guardar(destino, dados)
                ...
                path.unlink(missing_ok=True)
                if artefacto.get("id") in grimorio.get("reliquias_roubadas", []):
                    grimorio["reliquias_roubadas"].remove(artefacto["id"])
        ...
        guardar(Path("ordem_elfica/arquivo_missoes") / f"{missao['id']}.json", missao)
        eventos.append(missao)
