...
def inventariar_era(era, resumo_execucao):
    artefactos = listar_json("artifacts/relics")
    reflexos = listar_json("orders/black_squad/dark_library/corrupted_reflections")
    roubadas = listar_json("orders/black_squad/dark_library/stolen_relics")
    missions = listar_json("orders/elven_order/mission_archive")
    books = listar_json("artifacts/amulets/livros")
    lendas = ler("docs/lore/legends/livro_personagens_lendarias.json", {"personagens": []}).get("personagens", [])
    individuals = ler("datasets/generated/world_state/todos_individuos.json", [])
    ...
    save(BASE / "inventories" / f"inventario_era_{era:03d}.json", inventory)
    return inventory
...
def create_biographies(era, maximo=12):
    ...
    for pessoa in individuals:
        ...
        path = BASE / "biographies" / f"era_{era:03d}_{nome_seguro}.json"
        save(path, bio)
...
def create_museum(era, inventory):
    ...
    save(BASE / "museum" / f"museu_era_{era:03d}.json", museu)
    return museu
...
def write_chronicle(era, inventory):
    ...
    path = BASE / "chronicles" / f"cronica_era_{era:03d}.txt"
