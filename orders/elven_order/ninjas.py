...
from orders.black_squad.persistence import (
    REFLEXOS, ROUBADAS, load_grimoire, save_grimoire, ler, save
)
from artifacts.ark import save as save_relic
...
def execute_missions(config, ninjas, events):
    estado_path = Path("orders/elven_order/estado_ordem.json")
    ...
    for kind, path in alvos[:max_missoes]:
        ...
        if sucesso:
            ...
            if kind == "LIVRO":
                ...
                destino = Path("artifacts/amulets/cache/recovered_knowledge") / path.name
                save(destino, dados)
        ...
        save(Path("orders/elven_order/mission_archive") / f"{missao['id']}.json", missao)
