...
def filter_candidates(cands):
    ...
    for o,ch,p in cands:
        ...
        nova=normalize_candidate(ns,ch[1],random); events.append({'origem':o,'antes':ch,'depois':nova}); z.append((o+' [infectada]',nova,p))
    a=[]; rej=[]
    for o,ch,p in z:
...
def vote(cands):
    ...
    return {'chave':normalize_candidate([n for n,_ in vn.most_common(5)],[e for e,_ in ve.most_common(2)],random),'votos_numeros':vn.most_common(15),'votos_estrelas':ve.most_common(8)}
...
def corrupt(cfg,ch):
    ...
    return {'entidade':cfg['CORRUPTOR']['nome'],'chave_original':ch,'chave_corrompida':normalize_candidate(ns,es,random),'alteracoes_numeros':an,'alteracoes_estrelas':ae}
