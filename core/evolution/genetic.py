...
def execute(cfg,ctx):
    ...
    pop=[create(i+1) for i in range(tam)];contador=tam+1;todos={h.id:h for h in pop};cem=[];ress=[];resumo=[];regs=[];eventos_v=[];eventos_a=[];book_events=[];encontrados_persistentes=0
    for g in range(1,gens+1):
        ...
        for h in pop:
            ch=generate(h,ctx);an,ae,p,t=avaliar(ch,alvo);h.pontos+=p;h.titulo=t;h.keys.append({"geracao":g,"numeros":ch[0],"estrelas":ch[1]})
            regs.append({"geracao":g,"id":h.id,"nome":h.name,"classe":h.raca,"casa":h.casa,"numeros":ch[0],"estrelas":ch[1],"origem":"racas_antigas","virus":h.genoma.get("virus_malphas")})
        ...
        if cfg.getboolean("ARTEFACTOS_VIVOS","ativo",fallback=True):
            ...
            for h in elite[:3]:
                if random.random()<chance:
                    a=forjar(h,g,cfg,ctx.get('seed'));h.amuletos.append(a);eventos_a.append({"evento":"FORJA","geracao":g,"dono":h.name,"artefacto":a})
        ...
        resumo.append({"geracao":g,"melhor":elite[0].name,"raca":elite[0].raca,"pontos":elite[0].pontos,"eliminados":len(eliminados),"ressuscitados":len(regress)})
        if g==gens:pop=elite;break
        nova=elite+regress
        while len(nova)<tam:
            p1,p2=random.sample(elite,2);f=create(contador,g+1,[p1.id,p2.id]);contador+=1;f.raca=random.choice([p1.raca.replace("Renascido ",""),p2.raca.replace("Renascido ","")]);f.casa=random.choice([p1.casa,p2.casa]);todos[f.id]=f;nova.append(f)
        pop=nova
    return {"populacao_final":pop,"todos":todos,"cemiterio":cem,"ressuscitados":ress,"resumo":resumo,"registos":regs,"eventos_virus":eventos_v,"eventos_artefactos":eventos_a,"eventos_livros":book_events}
