...
def melforks(cfg, ctx):
    ...
    pop = [normalize_candidate(random.sample(range(1, 51), 5), random.sample(range(1, 13), 2), random) for _ in range(tam)]
    for _ in range(cfg.getint('MELFORKS', 'geracoes_chaves')):
        ...
        while len(nova) < tam:
            ...
            nova.append(normalize_candidate(random.sample(pn, 5), random.sample(pe, 2), random))
        pop = nova
